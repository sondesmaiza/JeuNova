...
$responsables = $pdo->query("SELECT u.id_user, u.nom, u.prenom FROM Utilisateur u JOIN Responsable r ON u.id_user = r.id_user")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE Evenement SET titre=?, type=?, description=?, date_debut=?, date_fin=?, capacite=?, is_actualite=?, duree=?, id_categorie=?, id_responsable=? WHERE id_event=?");
    $stmt->execute([$titre, $type, $description, $date_debut, $date_fin, $capacite, $is_actualite, $duree, $id_categorie, $id_responsable, $id]);
...
<div class="admin-card">
    ...
    <form method="post" class="row g-4">
        ...
        <div class="col-md-3">
            <select name="id_responsable" class="form-select form-control-premium">
                <option value="">-- Responsable --</option>
                <?php foreach($responsables as $r): ?>
                <option value="<?= $r['id_user'] ?>" <?= $event['id_responsable'] == $r['id_user'] ? 'selected' : '' ?>><?= htmlspecialchars($r['prenom'] . ' ' . $r['nom']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-3">
